...
func createFindFile(c *ishell.Context, folderName string, schemaName string, tableName string, modelName string, outputFields string, queryFields string, declareDateFields string, assignDateFields string) {
	////create find.go////
	findFile := []byte("")
	...
	appendToBytes(&findFile, fmt.Sprintf("func Find(where string, orderBy string)(%ss %ss,err error){\n", LcFirst(modelName), modelName))
